...
export async function GET(req: NextRequest) {
  ...
  try {
    const response = await organizationRepository.getRecentMembers(orgId);

    return NextResponse.json(
      { message: "success", members: response },
      { status: 200 }
    );
  } catch (error) {
    console.log("Something went wrong while trying to fetch recent members.");
    return NextResponse.json(
